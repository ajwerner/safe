...
class KeyChain:
    ...
    def __init__(self, path, namespace, password):
        '''
            ...
            if st.st_size == 0:
                ...
                for i in range(KeyChain.NR_DEVICES):
                    dt[i] = 0
                self.kc_file.write(struct.pack(tr(8 * KeyChain.NR_DEVICES)+"s", 
                                   buffer(dt)[:]))
                self.kc_file.flush()
        ...
        def read_keychain(self, key_type, dev_id=0):
            ...
            _offset += 4
            master_privkey = struct.unpack(str(master_privkey_len)+'s', 
                                           kc_mm[_offset:_offset + master_privkey_len])[0]
            ...
            if device_loc == 0:
                ...
                key = struct.unpack(str(key_len)+'s', 
                                    kc_mm[(device_loc + 4): (device_loc + 4 + key_len)])
            kc_mm.close()
            return key, None
